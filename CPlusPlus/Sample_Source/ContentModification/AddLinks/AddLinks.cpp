...
int main(int argc, char** argv)
{
    ...
    std::cout << "Adding 3 hyperlinks to file " << csInputFileName.c_str()
...
// Step 2) Create, set up, and place a link that will move to a new location.
        ...
        PDViewDestination nextDestination = PDViewDestCreate ( 
            inDoc.pdDoc, destPage,                     //The source document and destination page.
...
// Step 3) Create, set up, and place a link the will open a webpage.
        ...
        CosDictPut ( URIDict, 
                     ASAtomFromString("S"), 
                     CosNewName(inputDocCosDoc, false, ASAtomFromString("URI")));

        //Using the URI key "URI" set up the desired path
        CosDictPut ( URIDict, 
                     ASAtomFromString("URI"), 
                     CosNewString(inputDocCosDoc, false, "http://www.datalogics.com", strlen("http://www.datalogics.com")));
