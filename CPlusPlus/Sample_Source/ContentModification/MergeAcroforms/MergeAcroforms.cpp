...
DURING
    ...
    PDPageDrawContentsToMemory (InPage, kPDPageDoLazyErase, &Matrix, NULL,
                0, ColorSpaceName, 8, &Dest, (char *)BitMap, BitMapSize, NULL, NULL);
...
DURING
    ...
    PageImage = PDEImageCreate (&ImageAttrs, sizeof (PDEImageAttrs), &Matrix, 0, 
                    RGBColor, NULL, &Filters, NULL, BitMap, BitMapSize);
...
ASBool FieldDictCopy (CosObj KeyObj, CosObj Value, void* ClientData)
{
    ...
    if (Key == ASAtomFromString ("P")) 
    {
        ASInt32        Page;
        for (Page = 0; 
             ((!CosObjEqual (Control->InPageTable[Page], Value)) &&
              (Page < Control->Pages)); 
             Page++)
          ...
          ASInt32 PageCount, CosObj *InPageTable, CosObj *OutPageTable)
{
    ...
    if (CosObjGetType (Kids) != CosNull) 
    {
        ...
        for (Count = 0; Count < CosArrayLength (Kids); Count++)
        {
            ...
            CosObj  NewKid = CopyField (InDoc, OutDoc, Kid, NewField,
                        PageCount, InPageTable, OutPageTable);
...
int CopyAcroforms (PDDoc InDoc, PDDoc OutDoc)
{
    ...
    for (FieldNumber = 0; FieldNumber < CosArrayLength (InFields); FieldNumber++) {
        ...
        CosObj    OutField = CopyField (InCosDoc, OutCosDoc, InField, CosNewNull(),
                          PageCount, InPageTable, OutPageTable);
