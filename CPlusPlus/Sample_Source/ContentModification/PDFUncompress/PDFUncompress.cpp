...
int main (int argc, char* argv[])
{
    ...
    std::cout << "Will uncompress file " << csInputFile.c_str() << " and save as " << csOutputFile.c_str() << std::endl;
...
DURING
        DocCopy = PDDocCreate ();
        PDDocInsertPages (DocCopy, PDBeforeFirstPage, pdDoc, 0, PDAllPages, PDInsertAll, NULL, NULL, NULL, NULL);   
HANDLER
...
CosObj  Uncompress  (CosObj Stream)
{
    ...
    NewStream = CosNewStream (CosObjGetDoc (Stream), TRUE, Stm, 0, TRUE, 
                              CosDict, CosNewNull(), -1);
    ASStmClose (Stm);
