...
void SetupPDPrintParams(PDPrintParams psParams)
{
    ...
    psParams->shrinkToFit = true;                     // If the page is larger than the media, it will be evenly scaled down
                                                      // to fit the media (overrides scale parameter)
    psParams->psLevel = 3L;                           // PS Level - 1, 2, or 3
                                                      // Level of PostScript created. In practice, this should match the level of the printer.
                                                      // If the printer does not support the level specified, this will be replaced with the 
                                                      // highest level known to the printer.
    ...
    psParams->transparencyQuality = false;            // Echoed in PDFLPrintUserParams as the transQuaility attribute. 
                                                      ...
                                                      // as much as possible.
    psParams->farEastFontOpt = false;                 // Used to describe how large fonts should he handled, either downloaded, or not.
    ...
    psParams->rotateAndCenter = false;                // When true, indicate that the long edges of the page, and the media,
    ...
    psParams->centerCropBox = true;                   // When true, position the page such that the center of the page is at the
    ...
    psParams->duplex = kPDDuplexOff;                  // Simplex or duplex printed pages (single or double sided)
                                                      // This is also specified in the DevMode, for Print To PostScript or Print To GDI.
                                                      // In case of conflict, the DevMode value will be used.

    psParams->whichMarks = 0;                         // Page mark indication. It is a bit-wise OR of the PDPageMarkFlags values.
    psParams->markStyle = 0;                          // Specify the style to use for page marks. The PDPrintMarkStyles enum contains 
                                                      // possible styles.
    psParams->customMarksFileName = NULL;             // If markStyle == -1, this should be a valid file name pointing to a valid .mrk
                                                      // file for custom printer marks.
    ...
    psParams->hostBasedCM = false;                    // Do hostBased color management using the destination profile 
                                                      ...
                                                      // The default is false, which means doing CSA generation for profiles instead of
                                                      // converting all colors on the host.
    ...
    psParams->disableFlattening = false;              // When true, will cause transparency flattening to be bypassed when printing.
    psParams->doNotDownloadFauxFonts = false;         // When true, disables the downloading of fonts which are created to replace 
                                                      // fonts not directly usable.
    psParams->grayToK = false;                        // When true, will cause gray colors to be printed as CMYK colors,
                                                      // using only the black component.

    psParams->maxFlatSeconds = 0;                     // When nonzero, sets a limit to the amount of time APDFL will spend attempting
                                                      // to flatten a given page to vector art. If the time is exceeded, the page will
                                                      // be flattened to a bitmap image.

    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Attributes effective in both PostScript to a File, and PostScript to a Printer.
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    psParams->setPageSize = false;                    // When true, causes a PostScript setcachedevice command to be issued,
    ...
    psParams->incEmbeddedFonts = kIncludeOncePerDoc;  // Embed fonts that are embedded in the PDF file.
    ...
    // emitColorSeps, emitInRIPSeps and negative, can be used (Only when using PostScript to a Printer)
    // to print separation colors, rather than composite color pages.
    ...
    psParams->emitPSXObjects = false;                 // When true, will cause PDF Xobjects to be emitted to PostScript as PostScript XObjects.
                                                      // When false, they will be copied to each point of reference.
    psParams->useFontAliasNames = false;              // When true, uses the name of the font as it was supplied by the system, or subset,
    ...
    psParams->emitTransfer = true;                    // When true, will emit transfer functions to the PostScript document,
                                                      // rather than resolving their usage on the host.
    psParams->emitBG = true;                          // When true,  will emit Black Generation values to the PostScript file,
                                                      // rather than resolving their usage on the host.
    psParams->emitUCR = true;                         // When true, will emit Under Color Removal values to the PostScript file,
                                                      // rather than resolving their usage locally.

    psParams->suppressCJKSubstitution = false;        // If allowed, will control the use of printer based substitutes for CJK Fonts.
    psParams->brokenCRDs = false;                     // When true, disables the emission of profiled colors to PostScript.
                                                      // In essence, this will force Host color conversion.

    psParams->bitmapResolution = 0;                   // Set the resolution, in DPI, of emitted bitmaps
                                                      // when these are done as actual bitmaps.
    psParams->gradientResolution = 0;                 // Set the resolution, in DPI, of gradients interior to the object (not edges).
    ...
    psParams->suppressSnapToDevice = false;           // When true, will disable the “Snap To Device” rendering option.
                                                      // Snap To Device causes all elements to be pixel aligned at the printer resolution.
    psParams->suppressElement = 0;                    // Controls suppression of XMP data, written to the PostScript stream.
                                                      // Data may be controlled by type.
    ...
    psParams->printWhat = PDPrintWhat_DOCUMENT;       // Print the document, the document and comments, or document form fields only.
    ...
    psParams->suppressOPPWhenNoSpots = false;         // When true, will suppress OP processing of pages which contain no spot colors.
                                                      // This attribute may not be effective in all cases
                                                      // (That is, OP Processing may be done on some pages which do not contain spot colors,
                                                      // regardless of the setting of this flag).

    psParams->optimizeForSpeed = true;                // Emit PostScript such that all fonts are emitted once at the beginning
...
void SetupPDFLPrintUserParams(PDFLPrintUserParams userParams)
{
    ...
    userParams->emitToPrinter = true;                 // If emitToPrinter is true, then the printer identification information should be present.
    ...
    userParams->deviceNameW = NULL;                   // These may be supplied as either ASCII, or wide text strings.
    ...
    userParams->inFileNameW = NULL;                   // print driver, for use in a separator page (At the drivers discretion).
                                                      // When both are supplied, wide form is used.

    userParams->outFileName = NULL;                   // Used only when printing to a printer. They indicate that the final driver
    userParams->outFileNameW = NULL;                  // file should be saved in the named file, rather than passed to the printer
    ...
    userParams->pDevModeW = NULL;                     // The use of the wide form is preferable. If both are present, the wide form will be used.
    ...
    userParams->farEastFontOpt = false;               // Controls if very large fonts are to be downloaded or not. This is effective only for PostScript.
    ...
    userParams->startResult = 0;                      // Should be set to the jobId returned by StartDoc, if the application does the StartDoc.
    ...
    userParams->hDC = NULL;                           // Supplied by the user. This should be a Device Context for the same print device
                                                      // as is named in deviceName. If it is not, then deviceName will be replaced with the name of the
                                                      // device this context refers to.
    
    userParams->forceGDIPrint = false;                // When true, will cause Print To GDI to be used, even if the printer selected is capable of
                                                      // supporting PostScript.

#endif
    
    userParams->paperWidth = kPDPrintUseMediaBox;     // Used to select paper.  The value kPDPrintUseMediaBox will cause the page
                                                      // size to be used, rather than a fixed size.
    userParams->paperHeight = kPDPrintUseMediaBox;    // Used to select paper.  The value kPDPrintUseMediaBox will cause the page
                                                      // size to be used, rather than a fixed size.
    ...
    userParams->cancelProc = NULL;                    // Establish a procedure, which is called periodically, which is able to cancel a print operation
    userParams->clientData = NULL;                    // in progress.
    
    userParams->PPDFeatures = NULL;                   // Used only in printing to PostScript. Normally, the PPD File will be located via the system
    userParams->ppdFileName = NULL;                   // for PostScript to a Printer, and the features available will be only those features in the system
                                                      // installed PPD File. This mechanism may be used to attach feature information for PostScript to a File.


    userParams->progressProc = NULL;                  // Used to identify a procedure that should be called periodically during printing, to permit a
    ...
    userParams->doOPP = false;                        // Echoes the PDPrintParamsRec attribute of the same name. When true, we will “blend” CMYK colors
                                                      // using the OP transparency model.
#endif
}
