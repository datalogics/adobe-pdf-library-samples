...
typedef struct imagedef
{
    ...
    imagedef ( PDEImage img, ASBool mask, ASBool smask ) : m_inline ( false ), m_width ( 0 ), m_depth ( 0 ),
                               m_mask ( mask ), m_smask ( smask )
    {
...
std::ostream& operator<< ( std::ostream& os, const imagedef& id )
{
    ...
    if ( id.m_mask )
        os << " (Mask)";
    os << " is an " <<  ( id.m_inline ? "InLine" : "XObject" ) << " image " <<
          id.m_width << " pixels wide, and " << id.m_depth << " pixels deep. It is referenced "
          << id.m_refs.size() << " times.\n";
...
{
    int operator() ( int x, const ImageDef& id )
    {
        return x + id.m_refs.size();
    }
};
