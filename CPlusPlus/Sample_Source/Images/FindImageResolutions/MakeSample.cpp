...
template<class _Type> struct Aligner
{
    ...
    Aligner<_Type>& operator=(const _Type rhs) { m_data = rhs; return *this; }
    _Type m_data;
};
...
void CompleteImageTable (CosDoc doc)
{
    for (size_t count = 0; count < ImageTableSize; count++)
    {
        ...
        if (strstr (current->m_name, ".jpg"))
        {
            ...
            CosDictPutKeyString (jpegAttributes, "BitsPerComponent", CosNewInteger (doc, false, current->m_attrs.bitsPerComponent));
            CosDictPutKeyString (jpegAttributes, "Filter", CosNewNameFromString (doc, false, "DCTDecode"));

            current->m_cosImage = CosNewStream (doc, true, fileStm, 0, false, jpegAttributes, CosNewNull (), fileSize);
        ...
        else
        {
            ...
            CosDictPutKeyString (bmpAttributes, "BitsPerComponent", CosNewInteger (doc, false, current->m_attrs.bitsPerComponent));
            ...
            for (int count = 0; count < current->m_attrs.height; count++)
                memmove (&newImage[count * rowSize], &data[(current->m_attrs.height - count - 1) * unpackedRowSize], rowSize);

            ASStm bmpStm = ASMemStmRdOpen ((char *)newImage, P_biSizeImage);

            current->m_cosImage = CosNewStream (doc, true, bmpStm, 0, false, bmpAttributes, CosNewNull (), P_biSizeImage);
...
void InsertImage (PDEContent content, ASInt32 imageIndex, double X, double Y, 
                  double wide, double deep, double angle, ASBool mirrorH, ASBool mirrorV)
{
    PDEColorSpace rgbSpace = PDEColorSpaceCreateFromName (ASAtomFromString ("DeviceRGB"));
    ...
    PDEImage newImage = PDEImageCreateFromCosObjEx (&ImageTable[imageIndex].m_cosImage,
...
void InsertImageInForm (CosDoc doc, PDEContent content, ASInt32 imageIndex,
                        double X, double Y, double wide, double deep, double angle, ASBool sMask)
{
    ...
    PDEImage newImage = PDEImageCreateFromCosObjEx (&ImageTable[imageIndex].m_cosImage, &imageMatrix, rgbSpace, NULL);
