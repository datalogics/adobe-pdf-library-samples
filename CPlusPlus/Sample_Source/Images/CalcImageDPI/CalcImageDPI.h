...
std::ostream& operator<< ( std::ostream& o, const ASFixedMatrix& m )
{
    o << "[" << std::fixed << std::setprecision(2) << ASFixedToFloat(m.a) << ", " << ASFixedToFloat(m.b) << ", " << ASFixedToFloat(m.c) <<
         ", " << ASFixedToFloat(m.d) << ", " << ASFixedToFloat(m.h) << ", " << ASFixedToFloat(m.v) << "]";
    return o;
}
...
std::ostream& operator<< ( std::ostream& ofs, const imageentry& image )
{
    ...
    ofs << "        Image rotated " << image.hRotation << " Horiz and " << image.vRotation << " vertical." << std::endl;
    ofs << "        Erect H Matrix is " << image.erectHMatrix << " Erect V Matrix is " << image.erectVMatrix << std::endl;
    ofs << "        Image was " << image.widthInPoints << " Points wide and " << image.depthInPoints << " Points deep." << std::endl;
    ofs << "        Image was " << image.attrs.width << " Pixels wide and " << image.attrs.height << " pixels deep." << std::endl;
    ofs << "     for a DPI of " << image.hDPI << " horiz and " << image.vDPI << " vertical." << std::endl << std::endl;
    return ofs;
}
