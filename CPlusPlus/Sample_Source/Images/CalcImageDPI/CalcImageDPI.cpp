...
int main(int argc, char **argv)
{
    ...
    std::cout << "Analyzing file " << csInputFile.c_str() << ", writing results to " << csOutputFile.c_str() << std::endl;
...
DURING
        ...
        for (ASInt32 pageNumb = 0; pageNumb < pages; pageNumb++)
        {
            ...
            // Get the page's matrix. If we use unity, then we will have to "un rotate" all the upright
            // images in a rotated page (which we will do just fine). If we use the upright matrix, 
            // they they wil be unrotated automatically. Either works.
...
void FindImageDPI (PDEImage image,ASFixedMatrix *matrix, double userScale, ASInt16 pageNumb, ASInt16 sequence, std::vector<ImageEntry>& imageTable)
{
    ...
    idata.vRotation = atan2 (ASFixedToFloat (idata.finalMatrix.d), ASFixedToFloat (idata.finalMatrix.c)) / degrees_to_radians - 90;
    ...
    if ( (testX != testY) && (fabs (testX - testY) != 180) )
    {
        ...
        if (idata.erectHMatrix.b != 0)
        {
            ...
            idata.xShear =  -(atan2 (ASFixedToFloat (idata.finalMatrix.d), ASFixedToFloat (idata.finalMatrix.b)) / degrees_to_radians) + 90;
            memmove ((char *)&idata.erectHMatrix, (char *)&idata.finalMatrix, sizeof (ASFixedMatrix));
        ...
        {
            idata.yShear =  atan2 (ASFixedToFloat (idata.finalMatrix.c), ASFixedToFloat (idata.finalMatrix.a)) / degrees_to_radians;
...
void WalkContent (PDEContent content, ASFixedMatrix *matrix, double userScale, ASInt16 pageNumb, ASInt16 *sequence, std::vector<ImageEntry>& imageTable)
{
    if (!content)
        return;
