...
BOOL CPDFViewerApp::InitInstance ()
{
    ...
    */
    ...
    pDocTemplate = new CMultiDocTemplate (IDI_ICON1, RUNTIME_CLASS(CPDFViewerDoc), RUNTIME_CLASS(CDocumentFrame), RUNTIME_CLASS(CPDFViewerView));
...
int CMainFrame::OnCreate( LPCREATESTRUCT lpCreateStruct)
{
    ...
    */
    ...
    if (!SelectRotationList.Create (CBS_DROPDOWNLIST  | WS_VISIBLE |
                                 WS_TABSTOP | WS_VSCROLL, rect, &Toolbar, ID_PAGE_ROTATE))
    {
    ...
    if (!SelectScaleList.Create (CBS_DROPDOWN  | WS_VISIBLE |
                              WS_TABSTOP | WS_VSCROLL, rect, &Toolbar, ID_PAGE_SCALE))
    {
...
#define SelectPage100Percent 3
    ...
    */
    ...
    if (!SelectPageList.Create (CBS_DROPDOWN  | WS_VISIBLE |
                             WS_TABSTOP | WS_VSCROLL, rect, &Toolbar, ID_PAGE_SELECT))
    {
    ...
    */
    if (WaitForSingleObject (NextBitmapComplete, 0) == WAIT_TIMEOUT)
    {
        ...
        if (OldMapValid)
        {
            ...
            StretchBlt (GetDC()->m_hDC, leftIndent, topIndent, hSize, vSize, 
                        OldScreenDC, (ASUns32)(ScrollPosition.x * OldScale), (ASUns32)(ScrollPosition.y * OldScale), (int)(hSize * OldScale), (int)(vSize * OldScale), SRCCOPY);
            return;
        }
        else
...
//            WaitForSingleObject (NextBitmapComplete, INFINITE);
    ...
    */
    BitBlt (GetDC()->m_hDC, leftIndent, topIndent, hSize, vSize, 
                OffScreenDC, ScrollPosition.x, ScrollPosition.y, SRCCOPY);
}
...
*/
void CPDFViewerView::OnFirstPage()
{
    SetPage (0);
}
...
void CPDFViewerView::OnMouseMove (UINT flag, CPoint position)
{
    ...
    if (TrackMouse)
    {
        ...
        */
        if ((abs(ScrollPosition.x - NewScroll.x) > 10) ||
            (abs(ScrollPosition.y - NewScroll.y) > 10))
        {
            ScrollPosition = GetScrollPosition();
            OnDraw (GetDC());
        }
        ...
        for (ASUns32 row = 0; row < mapDeep; row++)
            for (ASUns32 col = 0; col < mapWide;  col++)
                memmove (&destData[(row * rowWideOut) + (col * bytesPerPixel)], 
                         &sourceData[((mapWide - col - 1) * rowWideIn) + (row * bytesPerPixel)], 
                         bytesPerPixel);
        break;
    case 180:
        /* Step through each pixel of each row, top to bottom,
    ...
    case 270:
        ...
        */
        for (ASUns32 row = 0; row < mapDeep; row++)
            for (ASUns32 col = 0; col < mapWide;  col++)
                memmove (&destData[(row * rowWideOut) + (col * bytesPerPixel)], 
                         &sourceData[(col * rowWideIn) + ((mapDeep - row -1) * bytesPerPixel)], 
                         bytesPerPixel);
        break;
    }
