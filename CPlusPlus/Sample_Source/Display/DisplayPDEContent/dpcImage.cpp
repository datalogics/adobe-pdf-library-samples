...
void DisplayImage (PDEImage Image, ASFixedMatrix *Matrix, PDEGraphicState *GState, ASBool HasGState)
{
    ...
    Outputter::Inst()->GetOfs() << ( IsMask ? "ImageMask" : "Image" ) << " of " << Attributes.width << " bits by "
                                << Attributes.height << " bits, " << Attributes.bitsPerComponent << " bits per "
    ...
    PDEFilterArray* Filters = (PDEFilterArray*)ASmalloc (sizeof(PDEFilterArray) + (NumberOfFilters * sizeof(PDEFilterSpec)));
    ...
    Outputter::Inst()->GetOfs() << sInLine.c_str() << " Image requires " << ImageRequiredSize << " bytes and is compressed to "
                                << ImageDataSize << "(" 
