...
void DisplayPath (PDEPath Path, ASFixedMatrix *Matrix, PDEGraphicState *GState, ASBool HasGState)
{
    ...
    Outputter::Inst()->GetOfs() << "Path Object: " << oss.str().c_str() << " At " << DisplayMatrix ( Matrix ).c_str() << std::endl;
    ...
    while (1)
    {
        ...
        switch (Operator)
        {
            ...
            case kPDELineTo:
                ...
                break;
            case kPDECurveTo:
                oss << "CurveTo " << AppendPoint (&One, Matrix, true).c_str() << AppendPoint (&Two, Matrix, true).c_str()
                    << AppendPoint (&Three, Matrix, false).c_str();
                Increment = 7;
                break;
            case kPDECurveToV:
                oss << "CurveToV " << AppendPoint (&One, Matrix, true).c_str() << AppendPoint (&Two, Matrix, false).c_str();
                Increment = 5;
                break;
            case kPDECurveToY:
                oss << "CurveToY " << AppendPoint (&One, Matrix, true).c_str() << AppendPoint (&Two, Matrix, false).c_str();
                Increment = 5;
                break;
            case kPDERect:
                oss << "Rectangle at  " << AppendPoint (&One, Matrix, true) << "Width: " << DisplayFixed(&Two.h)
                    << " Height: " << DisplayFixed (&Two.v);
                Increment = 5;
                break;
            case kPDEClosePath:
                oss << "Close Path";
