...
static void StartDisplayContent ( PDEContent Container, ASFixedMatrix *Matrix, PDEGraphicState *GState, ASBool HasGState)
{
    ...
    if (Attributes.flags & kPDESetCacheDevice)
    {
        ...
        outText << "Cache Values: [" << Amount1 <<  " " << Amount2 << " " << Amount3 
    ...
    {
        outText << "Contains Form type " << Attributes.formType << ". Rounding Box: " << DisplayRectangle ( Attributes.bbox ).c_str();
    ...
    Outputter::Inst()->GetOfs() << "Content Object: At " << DisplayMatrix ( Matrix ) << outText.str().c_str() << std::endl;
    Outputter::Inst()->GetOfs() << "{\n";
    return;
}

static void EndDisplayContent ()
{
    Outputter::Inst()->GetOfs() << "}" << std::endl;
}

static void StartDisplayContainer ( PDEContainer Container, ASFixedMatrix *Matrix, PDEGraphicState *GState, ASBool HasGState)
{
    Outputter::Inst()->GetOfs() << "Container Object: " << ASAtomGetString ( PDEContainerGetMCTag ( Container ) ) << std::endl;
    ...
    {
        Outputter::Inst()->Indent();
        Outputter::Inst()->GetOfs() << "Marked Content Dictionary: " << DisplayCosDict ( MCDict ).c_str() << std::endl;
        Outputter::Inst()->Outdent();
    }
    Outputter::Inst()->GetOfs() << "{" << std::endl;
    Outputter::Inst()->Indent();
...
static void EndDisplayContainer ( PDEContainer Container, ASFixedMatrix *Matrix, PDEGraphicState *GState, ASBool HasGState)
{
    Outputter::Inst()->Outdent();
    Outputter::Inst()->GetOfs() << "} End Marked Content Container " << ASAtomGetString ( PDEContainerGetMCTag ( Container ) ) << std::endl;
}
...
static void DisplayBeginContainer (PDEBeginContainer Container, ASFixedMatrix *Matrix, PDEGraphicState *GState, ASBool HasGState)
{
    Outputter::Inst()->GetOfs() << "Begin Container: " << ASAtomGetString ( PDEBeginContainerGetMCTag ( Container ) ) << std::endl;
