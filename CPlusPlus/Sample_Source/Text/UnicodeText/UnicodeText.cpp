...
int main(int argc, char** argv)
{
    ...
    std::cout << "Will write 2 pages of assorted Unicode strings to " << csOutputFileName.c_str() 
              << std::endl;
    ...
    for (int i = 0; i < NUM_TEXTS; ++i)                                                
    {
        for (int f = 0; f < 2; f++)                                                   
        {
            ...
            if (!PDEFontCheckASTextIsRepresentable(*nextFont, *textsAndFonts[i].text, &firstBadGlyph))
            {
                ...
                std::cout << "Error: The " << errString << "font associated with the" << i 
                          << "th text " << ASAtomGetString(badFontAttrs.name)
                          << " has no glyph for the text's " << firstBadGlyph << "th character." 
                          << std::endl;
...
// Step 4) Prepare to draw the texts to a new document.
    ...
    vertPlaceMatrix.v = horzPlaceMatrix.v = pageRect.top - Int16ToFixed(1 * 72);  // Y coordinate on page: 1" from the top.
...
// Step 5) Draw the texts, and subset the fonts we used.
    ...
    for (int i = 0; i < NUM_TEXTS; ++i)
    {
        ...
        PDETextAddASText(vertTexts, kPDETextRun, i, *textsAndFonts[i].text, 
                         *textsAndFonts[i].fontV, &graphics, sizeof(graphics), NULL, 0, &vertPlaceMatrix);
