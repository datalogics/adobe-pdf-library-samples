...
int main ( int argc, char* argv[] )
{
    ...
    ASUns8 *HelloWorldStr = (ASUns8*)"\x00\x48\x00\x65\x00\x6C\x00\x6C\x00\x6F\x00\x20\x65\xE5\x67\x2c\x00\x2E";
    //                                 H       e       l       l       o       [sp]    [Ni]    [Hon]   .  
#else
    size_t stringLen = 11;
...
DURING
...
#endif
    PDEFont pdeFont = PDEFontCreateFromSysFontAndEncoding ( sysFont, sysEncoding, pdeFontAttrs.name,
    ...
    ASFixedMatrix textMatrix = { 24 * fixedOne, fixedZero, fixedZero, 24 * fixedOne, fixedSeventyTwo, 2 * fixedSeventyTwo };
    PDEText pdeText = PDETextCreate();
    PDETextAdd ( pdeText, kPDETextRun, 0, HelloWorldStr, stringLen,
